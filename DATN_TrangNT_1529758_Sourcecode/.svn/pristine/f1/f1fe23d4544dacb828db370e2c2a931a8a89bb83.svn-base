...
<script type="text/javascript">
  function check(type) {
      ...
      if (type == 'save') {
          ...
          if (confirm('Bạn có chắc chắn muốn đệ trình KQPH Tín Phiếu này không?') == true) {
              if (guid != null && guid != "") {
                  $("#trang_thai").val("01");
                  f.action = 'TrinhKQPHTinPhieuAction.do?longid=<bean:write name="KQPHTinPhieuForm" property="guid"/>';
                  f.submit();
              }
          }
...
<html:form action="ViewKQPHTinPhieuAction" method="post">
          ...
          <logic:notEmpty name="KQPHTinPhieuForm" property="listCTSHTinPhieu">
            <logic:iterate id="objKQPHTinPhieu" name="KQPHTinPhieuForm"
              ...
              <tr class='<%=stt % 2 == 0 ? "trDanhSachChan" : "trDanhSachLe"%>'>
                ...
                <td class="left">
                  <bean:write  property="ma_chu_so_huu"
                             name="objKQPHTinPhieu" />
                </td>
                <td class="left">
  ...
  <div class="center">
    <logic:notEqual value="02" name="KQPHTinPhieuForm" property="trang_thai">
      <logic:notEqual value="01" name="KQPHTinPhieuForm" property="trang_thai">
        <button type="button" class="btn btn-default" onclick="check('save')"
                accesskey="r">
          Đệ t<span class="sortKey">r</span>&igrave;nh
        </button>
      </logic:notEqual>
    </logic:notEqual>
