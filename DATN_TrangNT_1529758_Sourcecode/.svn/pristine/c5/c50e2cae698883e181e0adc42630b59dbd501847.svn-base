...
public class StringUtil {
    ...
    public static Calendar StringToCalendar(String myString, String formatter) {
        if (myString == null) {
            return null;
        } else {
            SimpleDateFormat fm = new SimpleDateFormat(formatter);
    ...
    public static Calendar convertStringToDate(String strDate, String format) {
        ...
        if ("DD/MM/YYYY".equals(format)) {
            ...
            dElement = strDate.split("-");
            cal = Calendar.getInstance();
            cal.set(Calendar.YEAR, new Integer(dElement[0]).intValue());
            cal.set(Calendar.MONTH, new Integer(dElement[1]).intValue() - 1);
            cal.set(Calendar.DAY_OF_MONTH, new Integer(dElement[2]).intValue());
        }
    ...
    public static String getPreviousNextDate(int numberDay) {
        ...
        String resultDate = "";
        if (numberDay != 0) {
            resultDate = dateFormat.format(currentDate.getTimeInMillis() + (MILLIS_IN_DAY * numberDay));
        } else if (numberDay == 0) {
    ...
    public static String getPreviousNextDate(Calendar date, int numberDay) {
        ...
        String resultDate = "";
        if (numberDay != 0) {
            resultDate = dateFormat.format(date.getTimeInMillis() + (MILLIS_IN_DAY * numberDay));
        } else if (numberDay == 0) {
    ...
    public static String convertNumberToString(String number, String nt_id) {
        boolean nag = false;
        ...
        String decPart = "";
        if (k != -1) {
            numberStr = number.substring(0, k);
            decPart = number.substring(k + 1);
        }
        ...
        if (len < 4) {
            if (!"".equals(decPart)) {
                if (decPart.length() == 1)
                    decPart = decPart + "0";
                return numberStr + thapphan + decPart;
            }

        }
        firstPartIndex = len % 3;
        returnValue = numberStr.substring(0, firstPartIndex);
        remainPart = numberStr.substring(firstPartIndex);
        len = remainPart.length();

        for (int i = 0; i < len; i = i + 3) {
            returnValue = returnValue + hangnghin + remainPart.substring(i, i + 3);
        }
        if (firstPartIndex == 0) {
        ...
        if (!"".equals(decPart)) {
            if (decPart.length() == 1)
                decPart =   decPart + "0";
            returnValue = returnValue + thapphan + decPart;
        }
        return returnValue;
    }
    //KhangDQ
    ...
    public static String ConvertSize(String strLength) {
        ...
        if (intLength <= 500F) {
            ...
            if (intLength % 1024F == 0.0F)
                strSize = strSize.substring(0, strSize.indexOf(".")) + "&nbsp;Mb";
            else
                strSize = strSize.substring(0, strSize.indexOf(".") + 2) + "&nbsp;Mb";
        }
        return strSize;
    }
    ...
    public static String doubleToString(double dSo_tien) {
        ...
        if (iCommaPos <= 0) {
            ...
            if (iHieu_so == 0)
                ...
                sGia_tri_tra_ve =
                        sSo_tien.substring(0, iCommaPos) + sSo_tien.substring(iCommaPos + 1, iCommaPos + 1 + iSo_mu) + "." + sSo_tien.substring(iCommaPos + iSo_mu + 1, iEPos);
            }
        }
        sGia_tri_tra_ve = formatCurrency(sGia_tri_tra_ve);
        return sGia_tri_tra_ve;
    ...
    public static String pronounceVietnameseNumber(long lNumber) {
        String strUnit[] = { "", "ngh\354n", "tri\u1EC7u", "t\u1EF7", "ngh\354n t\u1EF7", "tri\u1EC7u t\u1EF7", "ngh\354n tri\u1EC7u t\u1EF7", "t\u1EF7 t\u1EF7" };
        ...
        for (int iUnitIndex = 0; iUnitIndex < strUnit.length && iUnitIndex * 3 < btDecimalCount; iUnitIndex++) {
            String str =
                pronounceVietnameseNumber(btDecimalNumber[iUnitIndex * 3], btDecimalNumber[iUnitIndex * 3 + 1], btDecimalNumber[iUnitIndex * 3 + 2], iUnitIndex * 3 + 2 < btDecimalCount);
            if (str.length() <= 0)
                continue;
    ...
    private static String pronounceVietnameseNumber(byte bUnit, byte bTen, byte bHundred, boolean bMax) {
        if (bUnit == 0 && bTen == 0 && bHundred == 0)
            return "";
        String strUnitSuffix[] = { "", "m\u1ED9t", "hai", "ba", "t\u01B0", "l\u0103m", "s\341u", "b\u1EA3y", "t\341m", "ch\355n" };
        String strUnitTen[] =
        { "", "m\u01B0\u1EDDi m\u1ED9t", "m\u01B0\u1EDDi hai", "m\u01B0\u1EDDi ba", "m\u01B0\u1EDDi b\u1ED1n", "m\u01B0\u1EDDi l\u0103m", "m\u01B0\u1EDDi s\341u",
            "m\u01B0\u1EDDi b\u1EA3y", "m\u01B0\u1EDDi t\341m", "m\u01B0\u1EDDi ch\355n" };
        String strUnit[] = { "", "m\u1ED9t", "hai", "ba", "b\u1ED1n", "n\u0103m", "s\341u", "b\u1EA3y", "t\341m", "ch\355n" };
        String strTenFirst[] =
        { "", "m\u01B0\u1EDDi m\u1ED9t", "hai m\u01B0\u01A1i m\u1ED1t", "ba m\u01B0\u01A1i m\u1ED1t", "b\u1ED1n m\u01B0\u01A1i m\u1ED1t", "n\u0103m m\u01B0\u01A1i m\u1ED1t",
            "s\341u m\u01B0\u01A1i m\u1ED1t", "b\u1EA3y m\u01B0\u01A1i m\u1ED1t", "t\341m m\u01B0\u01A1i m\u1ED1t", "ch\355n m\u01B0\u01A1i m\u1ED1t" };
        String strTen[] =
        { "", "m\u01B0\u1EDDi", "hai m\u01B0\u01A1i", "ba m\u01B0\u01A1i", "b\u1ED1n m\u01B0\u01A1i", "n\u0103m m\u01B0\u01A1i", "s\341u m\u01B0\u01A1i", "b\u1EA3y m\u01B0\u01A1i",
            "t\341m m\u01B0\u01A1i", "ch\355n m\u01B0\u01A1i" };
        String strHundred[] =
        { "kh\364ng tr\u0103m", "m\u1ED9t tr\u0103m", "hai tr\u0103m", "ba tr\u0103m", "b\u1ED1n tr\u0103m", "n\u0103m tr\u0103m", "s\341u tr\u0103m", "b\u1EA3y tr\u0103m",
            "t\341m tr\u0103m", "ch\355n tr\u0103m" };
        String strReturn = "";
        if (bMax || bHundred > 0)
    ...
    public static void main(String[] args) {
        try {
            ...
            Date d = new Date();
            Date dateToString = StringToDate("30/12/2018", "dd/MM/yyyy HH24:MM:SS");
            String dateToString2 = DateToString(d, "dd/MM/yyyy hh:mm:ss");
            Calendar ngay_tt_lai1 = StringUtil.convertStringToDate("04/02/2015", "DD/MM/YYYY");
            ngay_tt_lai1.set(Calendar.YEAR, ngay_tt_lai1.get(Calendar.YEAR) + 1);
            //System.out.println(StringUtil.convertCalendarToString(ngay_tt_lai1, "DD/MM/YYYY"));
            System.out.println(StringUtil.StringToDate("31/12/2018 20:00:00", "dd/MM/yyyy HH:mm:ss"));
            
          Calendar ngay_ph_bo_sung = StringUtil.convertStringToDate("30/12/2017", "DD/MM/YYYY");
          Calendar ngay_tt_lai12 = StringUtil.convertStringToDate("30/12/2017", "DD/MM/YYYY");
            
            System.out.println(ngay_ph_bo_sung.getTimeInMillis());
          System.out.println(ngay_tt_lai12.getTimeInMillis());
...
//                  Date date = sdf1.parse("30/12/2018 12:00:12");
...
//                  System.out.println("Current date in Date Format: " + string);
            ...
            //          Date d2 = StringToDate("12/06/2012 23:10:10", "dd/MM/yyyy HH:mm:ss");
                       System.out.println(new DecimalFormat("#,###0.00", DecimalFormatSymbols.getInstance(new java.util.Locale("vi", "VI"))).format(5.7));
        } catch (Exception e) {
            // TODO: Add catch code
            e.printStackTrace();
        }
    ...
    public static BigDecimal convertCurrencyToNumber(String strTien, String strLoaiTien) {
        strTien = strTien.replace(" ", "");
        if ("VND".equalsIgnoreCase(strLoaiTien) || "".equals(strLoaiTien) || strLoaiTien == null || "177".equals(strLoaiTien)) {
            strTien = strTien.replace(".", "");
        } else {
            strTien = strTien.replace(",", "");
        }
    ...
    public static String previousDateString(String dateString, String formatInput, String formatOutput) throws Exception {
    ...
    public static String convertCalendarToString(Calendar d, String format) {
        String dd = Integer.toString(d.get(Calendar.DATE));
        String mm = Integer.toString(d.get(Calendar.MONTH) + 1);
        String yyyy = Integer.toString(d.get(Calendar.YEAR));
        String hh = Integer.toString(d.get(Calendar.HOUR_OF_DAY));
    ...
    public static Date getFirstDayOfQuarter(int iQuy) {
        Date date = null;
        try {
            String sMonth = "";
            if (iQuy == 1) {
            ...
            String sTest = "01/" + sMonth + "/" + cal.get(Calendar.YEAR);
            date = dateFormat.parse(sTest);
            cal.setTime(date);
    ...
    public static Date getFirstDayOfQuarter(int iQuy, int year) {
        Date date = null;
        try {
            String sMonth = "";
            if (iQuy == 1) {
    ...
    public static Date getLastDayOfQuarter(int iQuy) {
        Date date = null;
        try {
            String sMonth = "";
            if (iQuy == 1) {
    ...
    public static Date getLastDayOfQuarter(int iQuy, int year) {
        Date date = null;
        try {
            String sMonth = "";
            if (iQuy == 1) {
