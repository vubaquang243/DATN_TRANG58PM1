package com.seatech.tp.lstruycap.action;
...
import com.seatech.tp.lstruycap.form.LSuTruyCapForm;
...
//        if (!checkPermissionOnFunction(request, "SYS.QLY_NSD.TRACUU_LSU_DANGNHAP")) {
...
//        }
        ...
        try {
            ...
            String strID_kb =
                session.getAttribute(AppConstants.APP_KB_CODE_SESSION)==null?"":session.getAttribute(AppConstants.APP_KB_CODE_SESSION).toString();

            //            b.kb_id in (select id from tp_dm_htkb where ma = ? or ma_cha = ?)
            v_param = new Vector();
            if (!strID_kb.equals("0001")) {
                if (!strID_kb.equals("0002")) {
                    strWhere =
                            " and b.kb_id in (select id from tp_dm_htkb where ma = ? or ma_cha = ?)";
