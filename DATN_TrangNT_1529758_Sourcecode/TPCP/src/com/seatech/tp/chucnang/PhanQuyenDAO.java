package com.seatech.tp.chucnang;
...
public class PhanQuyenDAO extends AppDAO {
    ...
    public int themQuyen(Long nNhomID, Long nCNangID) throws Exception {
        try {
            Long nID = getSeqNextValue("tp_cnang_nhom_seq", conn);
            String strSQL =
                "INSERT INTO  tp_cnang_nhom(id, cnang_id, nhom_id) " +
                "VALUES(?,?,?) ";
    ...
    public int xoaQuyen(Long nNhomID, Long nCNangID) throws Exception {
        try {
            String strSQL =
                "DELETE tp_cnang_nhom where cnang_id = ? and nhom_id = ? ";
    ...
    public int checkExist(Long nNhomID, Long nCNangID) throws Exception {
        ...
        try {
            String strSQL =
                "select count(*) counter from tp_cnang_nhom where cnang_id = ? and nhom_id = ? ";
    ...
    public int checkExistCNangCon(Long nNhomID,
      ...
      try {
          String strSQL =
              "select count(a.id) counter from tp_cnang_nhom a where a.nhom_id = ? and a.cnang_id in "+
              "( select b.id from tp_chucnang b where b.cnang_cha = ? )";
  ...
  public int deleteNhom_id(Long nNhomID) throws Exception {
      try {
          String strSQL =
              "DELETE tp_cnang_nhom where nhom_id = ? ";
          Vector vParam = new Vector();
          vParam.add(new Parameter(nNhomID, true));
                  return executeStatement(strSQL, vParam, conn);
      } catch (Exception e) {
