package com.seatech.tp.chucnang.action;
...
import com.seatech.tp.chucnang.form.PhanQuyenForm;
...
//            UserDAO uDAO = new UserDAO(conn);
            ...
            if (f.getNhom_id() != null) {
                ...
                strWhere =
                        " a.trang_thai = 'Y' and a.id in (select cnang_id from tp_cnang_nhom where nhom_id = ?)";
                ...
                vParam = new Vector();
                strWhere =
                        " a.trang_thai = 'Y' and (a.id not in (select a2.cnang_id from tp_cnang_nhom a2 where a2.nhom_id = ?) " +
                        //                         " or a.id in (select a3.cnang_cha from tp_chucnang a3 where a3.id not in "+
                        //                         " (select a4.cnang_id from tp_cnang_nhom a4 where a4.nhom_id = ?)))";
                        " or a.ma_cnang in (" + " select " + " case" +
                        " when MOD(a3.ma_cnang,10) <> 0 then a3.ma_cnang - MOD(a3.ma_cnang,10)" +
                        " else 0" + " end" +
                        " from tp_chucnang a3 where a3.id not in " +
                        " (select a4.cnang_id from tp_cnang_nhom a4 where a4.nhom_id = ?))" +
                        " or a.ma_cnang in (" + " select " + " case" +
                        " when MOD(a3.ma_cnang,100) <> 0 then a3.ma_cnang - MOD(a3.ma_cnang,100)" +
                        " else 0" +

                        " end" + " from tp_chucnang a3 where a3.id not in " +
                        " (select a4.cnang_id from tp_cnang_nhom a4 where a4.nhom_id = ?))" +
...
//            }
            ...
            saveVisitLog(conn, session,
                         "SYS.QLY_NHOM.PHAN_QUYEN","Thuc hien phan quyen cho nhom NSD");
