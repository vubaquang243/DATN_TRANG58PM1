package com.seatech.tp.ttindthau.vo;

import com.seatech.framework.datamanager.BaseVO;

public class ThongTinDauThauVO extends BaseVO {

    private String dot_dau_thau;

    private String dot_bo_sung;

    private String ma_tpcp;

    private String khoi_luong_tp;

    private String loai_tien;

    private String ky_han;

    private String ngay_to_chuc_ph;

    private String ngay_ph;
     
    private String nam_ngay_tt_lai; 
     
    private String ngay_tt_tien_mua;

    private String ngay_tt_lai;

    private String ngay_dao_han;

    private String phuong_thuc_thanh_toan;

    private String hinh_thuc_dt;

    private String pthuc_xacdinh_kqdt;

    private String so_tk_nhan;

    private String so_tbao_goi_thau;

    private String ngay_tbao_goi_thau;

    private String ghi_chu;

    private String trang_thai;

    private String nguoi_tao;

    private String ngay_tao;

    private String nguoi_sua_cuoi;

    private String ngay_sua_cuoi;

    private String nguoi_phe_duyet;

    private String ngay_phe_duyet;

    private String ly_do_tu_choi;

    private String ngay_tt_lai_1;

    private String ngay_tt_lai_2;

    private String menh_gia;
    
    private String menh_gia_chu;
    
    private String khoi_luong_tp_chu;
    
    private String khoi_luong_them;
    
    private String khoi_luong_them_chu;
    
    private String pageNumber;

    private String ky_tra_lai;

    private String tu_ngay;

    private String den_ngay;
   private String noi_dung_mail;

    private String ls_danh_nghia;
    private String ten_trang_thai;
   private String isAddmore;
    public ThongTinDauThauVO() {
        this.dot_dau_thau = "";
        this.dot_bo_sung = "";
        this.ma_tpcp = "";
        this.khoi_luong_tp = "";
        this.loai_tien = "";
        this.ky_han = "";
        this.ngay_to_chuc_ph = "";
        this.ngay_ph = "";
        this.ngay_tt_tien_mua = "";
        this.ngay_tt_lai = "";
        this.ngay_dao_han = "";
        this.phuong_thuc_thanh_toan = "";
        this.hinh_thuc_dt = "";
        this.pthuc_xacdinh_kqdt = "";
        this.so_tk_nhan = "";
        this.so_tbao_goi_thau = "";
        this.ngay_tbao_goi_thau = "";
        this.ghi_chu = "";
        this.trang_thai = "";
        this.nguoi_tao = "";
        this.ngay_tao = "";
        this.nguoi_sua_cuoi = "";
        this.ngay_sua_cuoi = "";
        this.nguoi_phe_duyet = "";
        this.ngay_phe_duyet = "";
        this.ly_do_tu_choi = "";
        this.ngay_tt_lai_1 = "";
        this.ngay_tt_lai_2 = "";
        this.menh_gia = "";
        this.ky_tra_lai = "";
        this.tu_ngay = "";
        this.den_ngay = "";
        this.ls_danh_nghia = "";
        this.ten_trang_thai = "";
        this.noi_dung_mail="";
    }


    public void setDot_dau_thau(String dot_dau_thau) {
        this.dot_dau_thau = dot_dau_thau;
    }

    public void setDot_bo_sung(String dot_bo_sung) {
        this.dot_bo_sung = dot_bo_sung;
    }

    public void setMa_tpcp(String ma_tpcp) {
        this.ma_tpcp = ma_tpcp;
    }

    public void setKhoi_luong_tp(String khoi_luong_tp) {
        this.khoi_luong_tp = khoi_luong_tp;
    }

    public void setLoai_tien(String loai_tien) {
        this.loai_tien = loai_tien;
    }

    public void setKy_han(String ky_han) {
        this.ky_han = ky_han;
    }

    public void setNgay_to_chuc_ph(String ngay_to_chuc_ph) {
        this.ngay_to_chuc_ph = ngay_to_chuc_ph;
    }

    public void setNgay_ph(String ngay_ph) {
        this.ngay_ph = ngay_ph;
    }

    public void setNgay_tt_tien_mua(String ngay_tt_tien_mua) {
        this.ngay_tt_tien_mua = ngay_tt_tien_mua;
    }

    public void setNgay_tt_lai(String ngay_tt_lai) {
        this.ngay_tt_lai = ngay_tt_lai;
    }

    public void setNgay_dao_han(String ngay_dao_han) {
        this.ngay_dao_han = ngay_dao_han;
    }

    public void setPhuong_thuc_thanh_toan(String phuong_thuc_thanh_toan) {
        this.phuong_thuc_thanh_toan = phuong_thuc_thanh_toan;
    }

    public void setHinh_thuc_dt(String hinh_thuc_dt) {
        this.hinh_thuc_dt = hinh_thuc_dt;
    }

    public void setPthuc_xacdinh_kqdt(String pthuc_xacdinh_kqdt) {
        this.pthuc_xacdinh_kqdt = pthuc_xacdinh_kqdt;
    }

    public void setSo_tk_nhan(String so_tk_nhan) {
        this.so_tk_nhan = so_tk_nhan;
    }

    public void setSo_tbao_goi_thau(String so_tbao_goi_thau) {
        this.so_tbao_goi_thau = so_tbao_goi_thau;
    }

    public void setNgay_tbao_goi_thau(String ngay_tbao_goi_thau) {
        this.ngay_tbao_goi_thau = ngay_tbao_goi_thau;
    }

    public void setGhi_chu(String ghi_chu) {
        this.ghi_chu = ghi_chu;
    }

    public void setTrang_thai(String trang_thai) {
        this.trang_thai = trang_thai;
    }

    public void setNguoi_tao(String nguoi_tao) {
        this.nguoi_tao = nguoi_tao;
    }

    public void setNgay_tao(String ngay_tao) {
        this.ngay_tao = ngay_tao;
    }

    public void setNguoi_sua_cuoi(String nguoi_sua_cuoi) {
        this.nguoi_sua_cuoi = nguoi_sua_cuoi;
    }

    public void setNgay_sua_cuoi(String ngay_sua_cuoi) {
        this.ngay_sua_cuoi = ngay_sua_cuoi;
    }

    public void setNguoi_phe_duyet(String nguoi_phe_duyet) {
        this.nguoi_phe_duyet = nguoi_phe_duyet;
    }

    public void setNgay_phe_duyet(String ngay_phe_duyet) {
        this.ngay_phe_duyet = ngay_phe_duyet;
    }

    public void setLy_do_tu_choi(String ly_do_tu_choi) {
        this.ly_do_tu_choi = ly_do_tu_choi;
    }

    public void setNgay_tt_lai_1(String ngay_tt_lai_1) {
        this.ngay_tt_lai_1 = ngay_tt_lai_1;
    }

    public void setNgay_tt_lai_2(String ngay_tt_lai_2) {
        this.ngay_tt_lai_2 = ngay_tt_lai_2;
    }


    public String getDot_dau_thau() {
        return dot_dau_thau;
    }

    public String getDot_bo_sung() {
        return dot_bo_sung;
    }

    public String getMa_tpcp() {
        return ma_tpcp;
    }

    public String getKhoi_luong_tp() {
        return khoi_luong_tp;
    }

    public String getLoai_tien() {
        return loai_tien;
    }

    public String getKy_han() {
        return ky_han;
    }

    public String getNgay_to_chuc_ph() {
        return ngay_to_chuc_ph;
    }

    public String getNgay_ph() {
        return ngay_ph;
    }

    public String getNgay_tt_tien_mua() {
        return ngay_tt_tien_mua;
    }

    public String getNgay_tt_lai() {
        return ngay_tt_lai;
    }

    public String getNgay_dao_han() {
        return ngay_dao_han;
    }

    public String getPhuong_thuc_thanh_toan() {
        return phuong_thuc_thanh_toan;
    }

    public String getHinh_thuc_dt() {
        return hinh_thuc_dt;
    }

    public String getPthuc_xacdinh_kqdt() {
        return pthuc_xacdinh_kqdt;
    }

    public String getSo_tk_nhan() {
        return so_tk_nhan;
    }

    public String getSo_tbao_goi_thau() {
        return so_tbao_goi_thau;
    }

    public String getNgay_tbao_goi_thau() {
        return ngay_tbao_goi_thau;
    }

    public String getGhi_chu() {
        return ghi_chu;
    }

    public String getTrang_thai() {
        return trang_thai;
    }

    public String getNguoi_tao() {
        return nguoi_tao;
    }

    public String getNgay_tao() {
        return ngay_tao;
    }

    public String getNguoi_sua_cuoi() {
        return nguoi_sua_cuoi;
    }

    public String getNgay_sua_cuoi() {
        return ngay_sua_cuoi;
    }

    public String getNguoi_phe_duyet() {
        return nguoi_phe_duyet;
    }

    public String getNgay_phe_duyet() {
        return ngay_phe_duyet;
    }

    public String getLy_do_tu_choi() {
        return ly_do_tu_choi;
    }

    public String getNgay_tt_lai_1() {
        return ngay_tt_lai_1;
    }

    public String getNgay_tt_lai_2() {
        return ngay_tt_lai_2;
    }

    public String getPageNumber() {
        return pageNumber;
    }

    public void setMenh_gia(String menh_gia) {
        this.menh_gia = menh_gia;
    }

    public String getMenh_gia() {
        return menh_gia;
    }

    public void setPageNumber(String pageNumber) {
        this.pageNumber = pageNumber;
    }

    public void setKy_tra_lai(String ky_tra_lai) {
        this.ky_tra_lai = ky_tra_lai;
    }

    public String getKy_tra_lai() {
        return ky_tra_lai;
    }

    public void setTu_ngay(String tu_ngay) {
        this.tu_ngay = tu_ngay;
    }

    public String getTu_ngay() {
        return tu_ngay;
    }

    public void setDen_ngay(String den_ngay) {
        this.den_ngay = den_ngay;
    }

    public String getDen_ngay() {
        return den_ngay;
    }

    public void setLs_danh_nghia(String ls_danh_nghia) {
        this.ls_danh_nghia = ls_danh_nghia;
    }

    public String getLs_danh_nghia() {
        return ls_danh_nghia;
    }

    public void setTen_trang_thai(String ten_trang_thai) {
        this.ten_trang_thai = ten_trang_thai;
    }

    public String getTen_trang_thai() {
        return ten_trang_thai;
    }

    public void setMenh_gia_chu(String menh_gia_chu) {
        this.menh_gia_chu = menh_gia_chu;
    }

    public String getMenh_gia_chu() {
        return menh_gia_chu;
    }

    public void setKhoi_luong_tp_chu(String khoi_luong_tp_chu) {
        this.khoi_luong_tp_chu = khoi_luong_tp_chu;
    }

    public String getKhoi_luong_tp_chu() {
        return khoi_luong_tp_chu;
    }

    public void setNoi_dung_mail(String noi_dung_mail) {
        this.noi_dung_mail = noi_dung_mail;
    }

    public String getNoi_dung_mail() {
        return noi_dung_mail;
    }

    public void setIsAddmore(String isAddmore) {
        this.isAddmore = isAddmore;
    }

    public String getIsAddmore() {
        return isAddmore;
    }

 

    public void setKhoi_luong_them_chu(String khoi_luong_them_chu) {
        this.khoi_luong_them_chu = khoi_luong_them_chu;
    }

    public String getKhoi_luong_them_chu() {
        return khoi_luong_them_chu;
    }

    public void setKhoi_luong_them(String khoi_luong_them) {
        this.khoi_luong_them = khoi_luong_them;
    }

    public String getKhoi_luong_them() {
        return khoi_luong_them;
    }

    public void setNam_ngay_tt_lai(String nam_ngay_tt_lai) {
        this.nam_ngay_tt_lai = nam_ngay_tt_lai;
    }

    public String getNam_ngay_tt_lai() {
        return nam_ngay_tt_lai;
    }
}
