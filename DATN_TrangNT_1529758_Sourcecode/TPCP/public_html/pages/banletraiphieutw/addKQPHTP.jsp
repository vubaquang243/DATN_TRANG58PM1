       ...
       var guid123 = $("#dot_bo_sung").val();
        $.ajax( {
          ...
          async : true, cache : false, success : function (data) {
              ...
              if(obj!=null){
              $("#ma_tpcp").val(obj.ma_tpcp);
               $("#ma_tpcp").attr("disabled",true);
              $('.selectpicker').selectpicker('refresh');
                var ma_tpcpval =obj.ma_tpcp;
              if (ma_tpcpval != null && ma_tpcpval != "") {
               if(ma_tpcpval.substr(0,4)=='TWBH'){
                $('#ma_dvi_so_huu_1').val('BH'); 
                $('#ma_dvi_so_huu_1').change();
                $('#ma_dvi_so_huu_1').attr("disabled", true);
               $('#extenstion').hide();
              }else if(ma_tpcpval.substr(0,4)=='TWSC'){
                 $('#extenstion').hide();
...
//      var strElement= str[1].substring(2,4);
      if(str.length  > 2 || str.length < 2){
        ...
        if( str[0].length <3 || str[0].length >3 ){
          alert("Bạn nhập đợt phát hành không đúng định dạng!");
          $("#dot_ph").focus();
          return false;
        }
        if( str[1].length <4 || str[1].length >4 ){
          alert("Bạn nhập đợt phát hành không đúng định dạng!");
          $("#dot_ph").focus();
          return false;
        }
      }
      var ngay_ph1 = $("#ngay_ph").val().split("/");
      if(str_dot_ph[1] != ngay_ph1[2]){
        alert("Bạn phải nhập đợt phát hành trùng với năm phát hành");
        return false;
      }
    ...
    var lai_suat_int= toNumber(lai_suat);
      if(lai_suat_int==0){
        alert("Bạn phải nhập lãi suât lớn hơn 0");
        $("#lai_suat").focus();
        return false;
      }
      ...
      if (ky_tra_lai1 == 2) {
         ...
         if(ngay_to_chuc_ph_cu!=""){
          var time_ngay_tt_lai_1= parseDate(ngay_to_chuc_ph_cu).getTime();
      ...
      if (v_ngay_tt_lai_1 != "" && v_ngay_tt_lai_1 != null && v_ngay_tt_lai_1 != "undefined") {
          ...
          var time_ngay_tt1= parseDate(ngay_thanh_toan).getTime();
          if(check1=="No"){
          if(time_ngay_tt1 >= time_ngay_tt_lai_12){
          ...
          var time_ngay_dao_han = parseDate(ngay_dao_han).getTime();
            if(time_ngay_dao_han <= time_ngay_tt_lai_12){
     ...
     if(check1=="No"){
      var time_ngay_tt_lai_123 =parseDate(v_ngay_tt_lai_1).getTime();
      if(time_ngay_tt_lai_123 <= time_ngay_ph){
    ...
    if(so_luong_tp==0){
     ...
     var so_luong_tp_int= toNumber(so_luong_tp);
      ...
      if (!checkDate_ngay_tt_tien_mua) {
       $("#ngay_tt_tien_mua").focus();
       alert('Bạn nhập sai định ngày thanh toán tiền mua trái phiếu !');
          return false;
      }
      return true;
  }
...
//      $("#dot_bo_sung").keydown(function(e){
...
//      })
      ...
      $("#ngay_tt_lai_2").datepicker( {
        ...
        $("#dot_bo_sung").change(function (e) {
          ...
          async : true, cache : false, success : function (data) {
              var obj = JSON.parse(data);
//              var ngay_dao_hanValue=obj.ngay_dao_han;
//              var datetime = new Date();
//              var ngay_dao_han_time= parseDate(ngay_dao_hanValue);
//              if(datetime < ngay_dao_han_time){
              if(obj!= null){
...
//              $("#ky_tt_lai").attr("disabled",true);
//              $('.selectpicker').selectpicker('refresh');
//              var time1= $("#ngay_ph").val();       
//              if(time1!=""){
//              
//                var timengay = parseDate(time1).getTime();
...
//                if(ngay_tt_lai_1=="" ){   
//                  
//                  document.getElementById("ngay_tt_tien_mua").value = time1;
//                  var ky_han1 = $('#ky_han').find(":selected").text().split(" ");
//                  var xValue =$("#ky_han").val();
//                  if(xValue!=''){
...
//                  var date1= new Date(yearLai2,month,day);
//                  var str = getFormattedDate(date1);
//                  $("#ngay_dao_han").val(str);
//                  }
//                }else{
...
//                  }
//                  var date2= new Date(yearLai,month,day);
//                  var str1 = getFormattedDate(date2);
//                  document.getElementById("ngay_dao_han").value = str1;
//                  document.getElementById("ngay_tt_tien_mua").value = time1;                 
//                }
//                }
//              if(obj.ky_tt_lai== "1"){
...
//      );
  ...
  function changDate(){
    var x = $("#ngay_ph").val();
    document.getElementById("ngay_tt_tien_mua").value= x;
        ...
        <div class="col-md-6">
              ...
              <div class="col-sm-7">
                <html:select styleClass="form-control selectpicker"
                 ...
                 <html:option value="">Vui l&ograve;ng chọn</html:option>
                  ...
                  <%--<html:hidden property="ky_han" styleId="ky_han"/>--%>
                </html:select>
              </div>
            </div>
          </div>
          ...
          <div class="col-md-6">
            <div class="form-group">
              ...
              <div class="col-sm-7">
                <html:text styleClass="form-control" name="BanLeTraiPhieuTwForm"
                           onkeypress="return validateFloatKeyPress(this,event);"
                           onkeydown="if(event.keyCode==13) event.keyCode=9;"
                           onfocus="textfocus(this);" 
                            tabindex="5" onblur="textlostfocus(this);"
          ...
          <logic:notEmpty name="BanLeTraiPhieuTwForm"
            ...
            <logic:iterate id="objKQBanLeChiTiet" name="BanLeTraiPhieuTwForm"
              ...
              <tr class='<%=stt % 2 == 0 ? "trDanhSachChan" : "trDanhSachLe"%>'>
                <td class="center">
                  <bean:write name="objKQBanLeChiTiet" property="stt"/>
                </td>
                <td class="left">
                ...
                <td class="center">
                  <html:text styleClass="form-control number" styleId="sl_dky_mua"
                             maxlength="20" onkeyup="formatNum(event.target);"
